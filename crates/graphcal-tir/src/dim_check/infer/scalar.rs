...
use graphcal_syntax::names::{FnName, GenericParamName, UnitName};
...
pub(super) fn infer_binop(
    ...
    match op {
        ...
        BinOp::Add | BinOp::Sub => {
            ...
            if let InferredType::Datetime(ls) = &lhs_type {
                let time_dim = Dimension::base(graphcal_syntax::dimension::BaseDimId::Prelude(
                    "Time".to_string(),
                ));
...
pub(super) fn infer_unary(
    op: &graphcal_syntax::ast::UnaryOp,
    operand: &Expr,
    declared_types: &HashMap<String, DeclaredType>,
    local_types: &HashMap<String, InferredType>,
    registry: &Registry,
...
pub(super) fn infer_convert(
    inner: &Expr,
    target: &graphcal_syntax::ast::UnitExpr,
    declared_types: &HashMap<String, DeclaredType>,
    local_types: &HashMap<String, InferredType>,
    registry: &Registry,
    builtin_fns: &HashMap<&str, graphcal_registry::builtins::BuiltinFunction>,
...
pub(super) fn infer_as_cast(
    expr: &Expr,
    inner: &Expr,
    target_type: &graphcal_syntax::ast::TypeExpr,
    declared_types: &HashMap<String, DeclaredType>,
    local_types: &HashMap<String, InferredType>,
    registry: &Registry,
    builtin_fns: &HashMap<&str, graphcal_registry::builtins::BuiltinFunction>,
    ...
    let resolved_target =
        crate::tir::resolve_type_expr(target_type, registry, no_dim_params, no_index_params, src)?;
    ...
    for (i, param) in type_def.generic_params.iter().enumerate() {
        if param.constraint != graphcal_registry::registry::TypeGenericConstraint::Unconstrained {
