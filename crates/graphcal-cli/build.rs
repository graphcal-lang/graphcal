...
fn rerun_if_changed(path: &Path) {
    // Optional VCS files may be absent in source checkouts or unpacked crates.
    // Cargo treats a watched missing file as dirty on every build.
    if path.exists() {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    rerun_if_changed(Path::new(".cargo_vcs_info.json"));
    let manifest = env::var_os("CARGO_MANIFEST_DIR").ok_or("CARGO_MANIFEST_DIR is not set")?;
    let out = env::var_os("OUT_DIR").ok_or("OUT_DIR is not set")?;
    engine::ensure(
        Path::new(&manifest),
        Path::new(&out),
