        ...
        DeclKind::Param(p) => {
            ...
            match &p.type_ann.kind {
                TypeExprKind::DimExpr(d) => {
                    ...
                    assert_eq!(d.terms[0].term.name.value.leaf_str(), "Length");
                }
                other => panic!("expected DimExpr, got {other:?}"),
            }
            assert!(matches!(
    ...
    match &file.declarations[0].kind {
        DeclKind::Param(p) => {
            ...
            match &p.type_ann.kind {
                TypeExprKind::DimExpr(d) => {
                    assert_eq!(d.terms.len(), 1);
                    assert_eq!(d.terms[0].term.name.value.leaf_str(), "Mass");
                }
                other => panic!("expected DimExpr, got {other:?}"),
            }
        ...
        DeclKind::Node(n) => {
            ...
            match &n.type_ann.kind {
                TypeExprKind::DimExpr(d) => {
                    ...
                    assert_eq!(d.terms[1].term.name.value.leaf_str(), "Time");
...
fn parse_derived_dimension() {
    ...
    match &file.declarations[0].kind {
        DeclKind::Dimension(d) => {
            ...
            assert_eq!(def.terms[0].term.name.value.leaf_str(), "Length");
            assert_eq!(def.terms[1].op, MulDivOp::Div);
            assert_eq!(def.terms[1].term.name.value.leaf_str(), "Time");
        }
...
fn parse_base_unit() {
    ...
    match &file.declarations[0].kind {
        DeclKind::Unit(u) => {
            ...
            assert_eq!(u.dim_type.terms[0].term.name.value.leaf_str(), "Length");
...
fn parse_type_decl_single_field() {
    // Record-shaped types are written as a single-variant tagged
    // union whose sole constructor's name matches the type's name.
    let source = "type Orbit { Orbit(sma: Length) }";
    let file = Parser::new(source).parse_file().unwrap();
    assert_eq!(file.declarations.len(), 1);
    match &file.declarations[0].kind {
        DeclKind::Type(u) => {
...
#[test]
fn parse_type_decl_empty_body_rejected() {
    // `type Eci {}` no longer parses: there is no zero-variant tagged
    // union. The author either meant `type Eci { Eci }` (unit marker)
    // or `type Eci;` (required, awaits include binding).
    assert!(Parser::new("type Eci {}").parse_file().is_err());
}
...
fn parse_type_decl_mixed_with_other_decls() {
    let source = r"
dim Velocity = Length / Time;
type TransferResult { TransferResult(dv1: Velocity, dv2: Velocity) }
param alt: Length = 400.0 km;
";
    let file = Parser::new(source).parse_file().unwrap();
...
#[test]
fn parse_type_decl_no_generics_unit_marker() {
    let source = "type Eci { Eci }";
    let file = Parser::new(source).parse_file().unwrap();
    match &file.declarations[0].kind {
        DeclKind::Type(u) => {
        ...
        DeclKind::Index(idx) => {
            ...
            match &idx.kind {
                IndexDeclKind::RequiredRange { dimension } => {
                    ...
                    assert_eq!(dimension.terms[0].term.name.value.leaf_str(), "Time");
                }
        ...
        DeclKind::Index(idx) => {
            ...
            match &idx.kind {
                IndexDeclKind::RequiredRange { dimension } => {
                    ...
                    assert_eq!(dimension.terms[0].term.name.value.leaf_str(), "Mass");
                    assert_eq!(dimension.terms[1].term.name.value.leaf_str(), "Length");
                    assert_eq!(dimension.terms[2].term.name.value.leaf_str(), "Time");
