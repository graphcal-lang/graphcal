...
use crate::syntax::ast::{Expr, ExprKind, FnBody};
use crate::syntax::visitor::ExprVisitor;

use crate::registry::error::GraphcalError;
use crate::registry::registry::{FnDef, FunctionRegistry};
...
pub fn check_no_recursion(
    ...
    let fn_spans: HashMap<&FnName, crate::syntax::span::Span> =
...
mod tests {
    ...
    fn check_recursion(source: &str) -> Result<(), GraphcalError> {
        ...
        let resolved = crate::ir::resolve::resolve(&file, &src).unwrap();
        ...
        for entry in &resolved.functions {
            builder.register_function(FnDef {
                ...
                generic_params: entry
                    ...
                    .generic_params
                    .iter()
                    .map(|g| crate::registry::registry::FnGenericParam {
                        name: g.name.value.clone(),
                        constraint: match g.constraint {
                            crate::syntax::ast::GenericConstraint::Dim => {
                                crate::registry::registry::FnGenericConstraint::Dim
                            }
                ...
                params: entry
                    ...
                    .params
                    .iter()
                    .map(|p| crate::registry::registry::FnParamDef {
