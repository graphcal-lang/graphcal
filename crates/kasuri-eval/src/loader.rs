...
use kasuri_syntax::ast::{DeclKind, File};
...
fn load_file_dfs(
    ...
    if !loading.insert(canonical_path.to_path_buf()) {
        ...
        return Err(CompileError::Eval(KasuriError::CircularImport {
            cycle: cycle_str,
        }));
    }
    ...
    let ast = kasuri_syntax::parser::Parser::with_name(&source, &name).parse_file()?;
...
fn io_not_found(path: &Path) -> CompileError {
    CompileError::Eval(KasuriError::FileNotFound {
...
mod tests {
    ...
    fn load_standalone_file() {
        let dir = setup_temp_dir(&[("standalone.ksr", "param x: Dimensionless = 1.0;")]);
        let project = load_project(&dir.path().join("standalone.ksr")).unwrap();
    ...
    fn load_simple_import() {
        let dir = setup_temp_dir(&[
            ("helper.ksr", "param y: Dimensionless = 2.0;"),
            (
                "main.ksr",
                "use \"./helper.ksr\" { y };\nnode z: Dimensionless = @y + 1.0;",
            ),
        ]);
        let project = load_project(&dir.path().join("main.ksr")).unwrap();
        assert_eq!(project.files.len(), 2);
        assert_eq!(project.load_order.len(), 2);
        // helper should be loaded before main (topological order)
        let helper_canonical = dir.path().join("helper.ksr").canonicalize().unwrap();
        let main_canonical = dir.path().join("main.ksr").canonicalize().unwrap();
    ...
    fn load_circular_import_detected() {
        ...
        let result = load_project(&dir.path().join("a.ksr"));
    ...
    fn load_missing_import_file() {
        let dir = setup_temp_dir(&[("main.ksr", "use \"./nonexistent.ksr\" { x };")]);
        let result = load_project(&dir.path().join("main.ksr"));
        assert!(result.is_err());
    }
    ...
    fn load_diamond_import_deduplication() {
        ...
        let dir = setup_temp_dir(&[
            ("d.ksr", "param w: Dimensionless = 4.0;"),
            (
                "b.ksr",
        ...
        let project = load_project(&dir.path().join("a.ksr")).unwrap();
        assert_eq!(project.files.len(), 4);
        // d should appear first in load order
        let d_canonical = dir.path().join("d.ksr").canonicalize().unwrap();
