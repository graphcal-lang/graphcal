...
mod tests {
    ...
    #[test]
    fn reserved_constructor_alias_diagnostic_points_at_the_local_alias() {
        let directory = tempfile::tempdir().unwrap();
        let source_dir = directory.path().join("src/app");
        std::fs::create_dir_all(&source_dir).unwrap();
        std::fs::write(
        ...
        .unwrap();
        std::fs::write(source_dir.join("lib.gcl"), "pub type Flag { Only }\n").unwrap();
        let source = "import app.lib::{ Only as E };\n";
        let root = source_dir.join("main.gcl");
        std::fs::write(&root, source).unwrap();
    ...
    #[test]
    fn time_scale_in_value_position_has_a_targeted_diagnostic() {
        let source = "node invalid: Dimensionless = UTC;";
        ...
        assert_eq!(
            diagnostic.code,
            Some(NumberOrString::String("graphcal::N018".to_string()))
        );
