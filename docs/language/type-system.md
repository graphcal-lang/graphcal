...
A regular fieldless tagged union (`type Foo { A, B }`) is NOT automatically an index. The `cat` keyword explicitly marks it as usable in `T[I]`, preventing accidental use of marker types as collection axes.
