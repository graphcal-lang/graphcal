...
対話型レポートでは、デスクトップ画面で入力の構造に応じたアウトラインと、独立してスクロールできる
結果ペインを並べて表示します。狭い画面では、2 つの窮屈なペインではなく、通常のページスクロールを
使用します。上部に追従する **Inputs** と **Results** ボタンで、下書き、ピン留め、結果タブの選択を
保持したまま移動できます。入力欄は行の幅いっぱいに表示し、操作対象をタッチしやすい大きさにします。
幅の広いプロットは指定された寸法を保持し、ページ全体を広げずに、キーボードフォーカス可能な専用領域内で
スクロールできます。スカラーと 1 フィールドのレコードは直接編集
