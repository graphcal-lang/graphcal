...
**プラグインのインポート。** インタラクティブなレポートは、ルートプロジェクトの Wasm
プラグインをマニフェストおよびロックファイルとともに埋め込みます。ネイティブ評価と同じ
実行量を計測するインタープリターでオフライン実行し、シグネチャの検査、ロックファイルの
ピン、fuel 制限、呼び出しの分離を維持します。プラグインはファイルシステムやネットワークに
アクセスできません。HTML を共有すると、埋め込まれたモデルのソースとプラグインのバイナリーも
共有されます。テキスト専用のプレイグラウンドエディターは、このバイナリーバンドルを受け付けません。

レポートバンドルの上限は 4096 成果物、成果物ごとに 16 MiB、デコード後の内容の合計が
64 MiB（JSON エンベロープは 96 MiB）です。既存のローダーとプラグインの制限も適用されます。
無効または上限を超えるバンドルは、黙って静的レポートになるのではなく明示的に失敗します。

パッケージ依存関係はインタラクティブなレポートではまだサポートされず、ビルド時に明示的に
失敗します。そのようなモデルには `--static` を使用してください。ベースラインはプラグインを
含めて引き続きネイティブに計算されます。
