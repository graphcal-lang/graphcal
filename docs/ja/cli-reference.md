...
キー入力は未適用の下書きだけを変更し、最後に受け付けられた結果を表示し続けます。**Apply** は
変更していないフィールドも含むパラメーター全体を検査してアトミックに置き換え、**Discard edits** は
開始時のスナップショットを復元します。新しく選んだコンストラクターの必須フィールドは空で始まり、
ゼロ、false、単位、既定値を捏造しません。Apply に成功すると、埋め込まれたエンジン（この CLI と同じ
コンパイラーと評価器を WebAssembly にコンパイルし、Web Worker で実行するもの）が再評価を行い、
値、グリッド、バッジ、チャートをその場で更新します。静的な値カードとハイドレートされた
...
幅の広いグリッドや長いフィールドパスが隣のカードと重なることはありません。インデックス付き
エディターは完全な下書きを保持したまま最初に 32 要素ずつマウントし、**Show more entries** で残りを
順次表示します。結果領域にフォーカスし、矢印キーを使うと、はみ出した列を確認できます。再計算時も
領域のフォーカスとスクロール位置は
保持されます。画面上では、これらの領域の高さは最大 24 rem（またはビューポートの高さの 60%）に
制限され、短いカードが隣の高いカードに合わせて引き伸ばされることはありません。スクロールすると
すべての行を確認でき、値は切り捨てられません。**Report sections** ナビゲーションから、入力、値、
プロット、チェック、診断（存在する場合）、来歴へ直接移動できます。印刷時は高さ制限が解除され、
...
スナップショットを再読み込みできます。Apply すると明示的なパラメーター全体の閲覧者束縛が
作成されます。**Use default** は、その束縛（ビルド時の上書きを含む）を除いた完全な束縛集合を
