...
幅の広いグリッドや長いフィールドパスが隣のカードと重なることはありません。ハイドレートされた
入力カードでは、受け付け済み値のプレビューを 3 rem、エディターを 14 rem（またはビューポートの
高さの 35%）までに抑え、完全な内容はスクロールで確認できます。そのため、編集しながら結果を近くに
表示できます。インデックス付きエディターは完全な下書きを保持したまま最初に 32 要素ずつマウントし、**Show more entries** で残りを
