...
編集内容は **Apply** がパラメーター全体を検査して置き換えるまで下書きに留まります。**Discard edits** は受け付け済みのスナップショットを復元し、高度な完全値入力には **Raw literal** も利用できます。無効な入力には該当するフィールドまたは要素でエラーを表示し、最後に成功した計算結果を表示し続けます。
