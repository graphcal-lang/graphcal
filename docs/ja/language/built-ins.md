...
付きの 1 引数 `datetime` を使用してください。廃止された位置引数形式
`epoch("...", TT)` は拒否されます。`epoch<TT>("...")` と書いてください。
...
引用符で囲まれたタイムゾーンの綴りは文脈依存の `TimezoneLiteral` であり、
第一級の `String` 値ではありません。これらはプログラムの検査時に検証および
正規化され、その後は不透明な IANA 識別子としてコンパイルと評価を通じて
引き継がれます。

タイムゾーンの構築、検証、表示はすべて、Graphcal ツールチェーンに同梱された
同一の IANA タイムゾーンデータベースを使用します。Graphcal はホストの zoneinfo
インストールを参照しないため、特定の Graphcal リリースは、サポートされる
すべてのプラットフォームでタイムゾーン規則を同一に解決します。無効な
タイムゾーンの診断には、同梱された IANA tzdb のリリースが報告されます。
これらの規則の更新は、明示的な Graphcal の依存関係更新として行われます。
...
高コストなカーネルが実行される前に明示的に失敗します。長時間のカーネルは、
有界の間隔で協調的なキャンセルも監視するため、LSP とブラウザーでの評価の
応答性が保たれます。
