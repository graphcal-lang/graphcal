...
複数のリビジョンが共存できます。このファイルは手で編集せず、`graphcal deps lock` で生成してください。
...
`graphcal deps lock` は、ソースハッシュが検証済みのロックと一致する場合、
再取得せずにキャッシュ済みのチェックアウトを再利用します。
`GRAPHCAL_CACHE_DIR` でキャッシュルートを変更できます。

ソースハッシュの対象は `graphcal.toml`、宣言されたソースディレクトリ、
およびそのソースがインポートするすべての Wasm プラグインです。ソースディレクトリ外の
プラグインも含みます。パッケージのパスはパッケージルート内に収まり、UTF-8 の名前を
使う必要があります。シンボリックリンクと特殊ファイルはサポートされません。

プロジェクトの読み込み上限は、`.gcl` ソース、ソースツリーファイル、Wasm プラグインごとに
16 MiB、マニフェストごとに 1 MiB、ロックファイルごとに 4 MiB です。
成果物／ソースツリーエントリーの合計は 10,000 個、256 MiB までです。
上限を超えるとローダーエラーになります。
...
アサーションを、ネストされた include のアサーションも含めて評価します。
