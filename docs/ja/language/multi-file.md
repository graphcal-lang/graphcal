...
対象は `graphcal.toml`、宣言されたソースディレクトリ、およびそのソースがインポートする
すべての Wasm プラグイン成果物です。ディレクトリ外の成果物も含みます。ソースとプラグインの
バイト列を一度だけ取得し、検証と評価ではその不変スナップショットを使用します。
この範囲外の無関係なファイルは、ハッシュ化も実行もされません。
