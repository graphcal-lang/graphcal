...
!!! note "空のインデックスは存在しません"
    ...
    ありません。内部で空の値が集約に到達した場合、Graphcal はコンパイラー不変条件違反として
    `X001` を報告します。空の集約にはユーザー向けの意味論はありません。
...
現在、追加の軸を持てるスロットは最大 1 つです。複数の隣接する追加軸スロットは、将来の拡張として計画されています。
...
多重宣言の各スロットは、ナビゲーションの観点ではそれ自身が独立した宣言です。`gotoDefinition`、`findReferences`、`rename`、`hover` はすべてスロットヘッダーに着地し、各スロットはその名前にそれ自身のインレイヒントを受け取ります。軸、文脈依存ヘッダーラベル、修飾スライス参照もナビゲーションとリネームに参加します。フォーマッターは多重宣言の表層形式を保持しつつ、整列を正規化し、必要なすべての軸修飾子を保持します。セルレベルのインレイヒント(スロット名をソースのヘッダー行に投影するもの)は今後の課題です。
