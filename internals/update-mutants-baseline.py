...
def apply_campaign_plan(
    ...
    for mutation_id in _identity_list(value.get("obsolete"), "plan.obsolete"):
        ...
        if existing is not None and existing.status is not FindingStatus.OBSOLETE:
            findings[mutation_id] = existing.mark_obsolete()
    ...
    if audit_complete and audited:
        ...
        outcomes = {
            outcome.mutation_id: outcome.summary for outcome in load_outcomes(audit_report)
        }
        for mutation_id in audited:
            existing = findings.get(mutation_id)
            if existing is None:
                raise ValueError(f"audited mutation is absent from baseline: {mutation_id}")
            match outcomes.get(mutation_id):
                case "MissedMutant" | "Timeout":
                    _open_finding(findings, mutation_id)
                case "CaughtMutant":
                    findings[mutation_id] = existing.resolve(Resolution.CAUGHT)
                case "Unviable":
                    findings[mutation_id] = existing.resolve(Resolution.UNVIABLE)
                case None:
                    print(
    ...
    if discovery_report.is_file():
        for outcome in load_outcomes(discovery_report):
            if outcome.summary in FINDING_SUMMARIES:
                _open_finding(findings, outcome.mutation_id)
...
def main() -> int:
    ...
    for plan in plans:
        try:
            apply_campaign_plan(plan, findings)
            processed += 1
