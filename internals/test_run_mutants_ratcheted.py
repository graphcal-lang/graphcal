...
class MutationRunnerTest(unittest.TestCase):
    def test_partial_candidate_filters_disable_obsolete_detection(self) -> None:
        self.assertTrue(runner._has_partial_candidate_filter(["-F", "example"]))
        self.assertTrue(runner._has_partial_candidate_filter(["--shard=1/2"]))
        self.assertFalse(runner._has_partial_candidate_filter(["-f", "crate.rs"]))
