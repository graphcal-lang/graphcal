...
PARTIAL_CANDIDATE_OPTIONS = frozenset(
    {"-D", "-E", "-F", "--exclude-re", "--in-diff", "--re", "--shard"}
)


def _has_partial_candidate_filter(arguments: list[str]) -> bool:
    cargo_options = arguments[: arguments.index("--")] if "--" in arguments else arguments
    return any(
        argument in PARTIAL_CANDIDATE_OPTIONS
        or any(
            argument.startswith(f"{option}=")
            for option in PARTIAL_CANDIDATE_OPTIONS
            if option.startswith("--")
        )
        for argument in cargo_options
    )
...
def main(
    ...
    *,
    ...
    obsolete = (
        set(scoped_baseline) - set(all_candidates)
        if not _has_partial_candidate_filter(cargo_arguments)
        else set()
    )
    open_candidates = [
