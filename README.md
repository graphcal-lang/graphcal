...
- **Algebraic types and generics.** One- or multi-constructor `type` declarations, exhaustive `match`, and generic types with phantom parameters for things like reference frames (`Vec3<Length, Eci>` vs `Vec3<Length, Body>`).
- **Indexed values.** First-class index sets, `for` comprehensions, aggregations (`sum`, `mean`, ...), and `unfold` for time-series and recurrences. Spreadsheet-style tables keep ordinary row/column labels bare while preserving full qualification for slice and heterogeneous-header coordinates. Recurrence and range words are contextual, so engineering names such as `step` remain available as ordinary identifiers elsewhere.
