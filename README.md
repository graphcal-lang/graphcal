...
- **Reusable computation.** `dag` blocks parameterize sub-graphs and instantiate them as expressions or via `include`; required `pub(bind)` dimensions, types, and indexes act as explicit type-level input ports for dimension- and axis-polymorphic libraries. Inline DAG bodies use explicit `import`/`include` edges just like file DAGs. Multi-file projects compose with module-qualified type/index/constructor paths, explicit `pub` / `pub(bind)` exports, and separately modeled param input ports.
