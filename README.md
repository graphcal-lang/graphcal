...
- **Indexed values.** First-class index sets, `for` comprehensions, aggregations (`sum`, `mean`, ...), and `unfold` for time-series and recurrences. Recurrence and range words are contextual, so engineering names such as `step` remain available as ordinary identifiers elsewhere.
